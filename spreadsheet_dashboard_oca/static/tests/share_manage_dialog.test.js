...
test("copies the share link when the clipboard API is unavailable", async () => {
    // Navigator.clipboard is not exposed on non-secure origins (HTTP runboat).
    patchWithCleanup(browser.navigator.clipboard, {writeText: undefined});
    let copiedText = null;
    patchWithCleanup(browser.document, {
        execCommand(command) {
            copiedText = browser.document.querySelector("textarea").value;
            return command === "copy";
        },
    });
    await mountWithCleanup(ShareLinkCopyButton, {props: {content: SHARES[0].full_url}});
    await contains(".o_clipboard_button").click();
    expect(copiedText).toBe(SHARES[0].full_url);
});
